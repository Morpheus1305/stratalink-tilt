...
function buildVelocitySeries(depthSeries: Point[]): SparkPoint[] {
  ...
  for (let i = 1; i < depthSeries.length; i++) {
    ...
    result.push({ v: perMin / 1_000_000 });
  }
...
export default function LiquidityVelocitySparkline() {
  ...
  const latest = velocitySeries.length
  ...
  return (
    <div className="flex flex-col">
      ...
      <Sparkline
        ...
        barEnabled={false}
        stale={stale}
        yFormatter={(v: number) => `${v.toFixed(1)}M`}
        labelFormatter={(v: number) => `${v.toFixed(2)}M / min`}
        zeroLine={true}
      />
    </div>
  );
