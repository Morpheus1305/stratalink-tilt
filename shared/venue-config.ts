...
export const VENUE_CONFIGS: Record<string, VenueConfig> = {
  ...
  securitize: {
    venue: "SECURITIZE",
    displayName: "Securitize Markets",
    role: "REFERENCE_ADJACENT",
    confidence: "HIGH",
    scope: ["SPOT"],
    available: true,
    description: "SEC-registered ATS and transfer agent for tokenized real-world assets. Primary venue for BlackRock BUIDL and Ondo OUSG/ACRED. Institutional-only onboarding with KYC/AML at the smart-contract level.",
    usedFor: [
      "RWA depth anchoring",
      "Tokenized fund NAV cross-validation",
      "Regulated ATS liquidity reference",
      "PoLi+ attestation input",
    ],
    notUsedFor: [
      "Stress discovery",
      "Derivatives or leverage analysis",
      "Retail flow modeling",
    ],
  },
  archax: {
    venue: "ARCHAX",
    ...
    scope: ["SPOT"],
    available: true,
    description: "FCA-regulated digital asset exchange and custodian (UK). Primary European venue for tokenized money market funds including BENJI, VBILL, BCSPX, and BIB01. Institutional-grade settlement with T+0 tokenized clearing.",
    usedFor: [
      "European RWA liquidity reference",
      "Tokenized MMF depth anchoring",
      "FCA-regulated venue cross-validation",
      "PoLi+ attestation input",
    ],
    notUsedFor: [
      "Stress discovery",
  ...
  inx: {
    venue: "INX",
    displayName: "INX Digital",
    role: "REFERENCE_ADJACENT",
    confidence: "MODERATE",
    scope: ["SPOT"],
    available: true,
    description: "SEC- and FINRA-registered broker-dealer and ATS for security tokens. First regulated security token IPO. Covers ONDO and BUIDL with institutional-grade settlement.",
    usedFor: [
      "US ATS security token liquidity",
      "ONDO institutional depth reference",
      "Regulated broker-dealer flow signals",
    ],
    notUsedFor: [
      "Stress discovery",
      "Derivatives or leverage analysis",
      "DEX or DeFi analysis",
    ],
  },
  tzero: {
    venue: "TZERO",
    ...
    scope: ["SPOT"],
    available: true,
    description: "SEC-registered ATS (subsidiary of Overstock/Medici). Pioneer in security token trading infrastructure. Covers ONDO and BUIDL with DTC-connected settlement.",
    usedFor: [
      "US ATS security token liquidity",
      "DTC-connected settlement monitoring",
      "ONDO liquidity cross-validation",
    ],
    notUsedFor: [
      "Stress discovery",
      "Derivatives or leverage analysis",
  ...
  sdx: {
    ...
    scope: ["SPOT"],
    available: true,
    description: "SIX Digital Exchange — regulated CSD and trading venue for digital securities under FINMA/Swiss law. Covers tokenized equities (BCSPX) and bonds (BIB01) with Swiss CSD settlement and full DvP.",
    usedFor: [
      "Swiss regulated digital securities depth",
      "Tokenized equity and bond anchoring",
      "FINMA-supervised venue reference",
      "PoLi+ attestation input",
    ],
    notUsedFor: [
      "Stress discovery",
  ...
  addx: {
    ...
    scope: ["SPOT"],
    available: true,
    description: "MAS-regulated private exchange (Singapore) for tokenized funds, private credit, and structured notes. Strong Asia-Pacific institutional presence. Covers BUIDL, OUSG, BENJI, and USDY.",
    usedFor: [
      "APAC RWA institutional depth",
      "Yield-bearing stablecoin (USDY) anchoring",
      "MAS-regulated venue cross-validation",
      "PoLi+ attestation input",
    ],
    notUsedFor: [
      "Stress discovery",
