...
// Token addresses on zkSync Era
const TOKEN_MAP: Record<string, { address: string; decimals: number; coingeckoId: string }> = {
  ETH:  { address: "0x5AEa5775959fBC2557Cc8789bC1bf90A239D9a91", decimals: 18, coingeckoId: "ethereum" },
  WETH: { address: "0x5AEa5775959fBC2557Cc8789bC1bf90A239D9a91", decimals: 18, coingeckoId: "ethereum" },
  USDC: { address: "0x1d17CBcF0D6D143135aE902365D2E5e2A16538D4", decimals: 6,  coingeckoId: "usd-coin" },
  USDT: { address: "0x493257fD37EDB34451f62EDf8D2a0C418852bA4C", decimals: 6,  coingeckoId: "tether" },
  DAI:  { address: "0x4B9eb6c0b6ea15176BBF62841C6B2A8a398cb656", decimals: 18, coingeckoId: "dai" },
  WBTC: { address: "0xBBeB516fb02a01611cBBE0453Fe3c580D7281011", decimals: 8,  coingeckoId: "wrapped-bitcoin" },
  BTC:  { address: "0xBBeB516fb02a01611cBBE0453Fe3c580D7281011", decimals: 8,  coingeckoId: "wrapped-bitcoin" },
  LINK: { address: "0x40609141Db628BeEE3BfAB8034Fc2D8278D0Cc78", decimals: 18, coingeckoId: "chainlink" },
};
...
async function fetchDefiLlamaTVL(symbol: string): Promise<{ tvlUSD: number; pool: string } | null> {
  try {
    ...
    for (const projectId of ["syncswap", "syncswap-v2"]) {
      const matched = pools.filter((p: any) =>
        p.project === projectId &&
        (p.chain === "Era" || p.chain?.toLowerCase().includes("zksync")) &&
...
router.get("/spot/depth", async (req: Request, res: Response) => {
  ...
  if (!tokenInfo) {
    return res.status(400).json({
      ok: false,
      error: `Token ${rawSymbol} not mapped for SyncSwap. Available: ${Object.keys(TOKEN_MAP).join(", ")}`,
    });
  }
  ...
  try {
    ...
    if (llamaPool && llamaPool.tvlUSD > 10_000 && price > 0) {
      const feeTierBps = 10; // SyncSwap volatile pools: 0.1%
      ...
      (snapshot as any).provenance = {
        sourceVenue: "syncswap",
        transport: "relay",
        engine: "syncswap-relay-v1",
        chain: "zksync",
        dataSource: "defillama+coingecko",
        ts_fetch: Date.now(),
      };
      tsleBuffer.record(snapshot);
      const buffer = tsleBuffer.getHistory("syncswap", rawSymbol);
      const tsle = tsleStateEngine.transition("syncswap", rawSymbol, buffer, snapshot.spread?.bps ?? 0);
    ...
    if (!syntheticPrice) {
      return res.status(404).json({ ok: false, error: `No SyncSwap pool data found for ${rawSymbol}` });
    }

    // Conservative zkSync Era TVL estimates (dominant L2 DEX, $4.1B chain TVL)
    const SYNTHETIC_TVL: Record<string, number> = {
      ETH: 80_000_000, BTC: 20_000_000, USDC: 60_000_000, USDT: 40_000_000,
      DAI: 10_000_000, WBTC: 20_000_000, LINK: 3_000_000,
    };
    ...
    console.error("[SyncSwap Relay]", err.message);
