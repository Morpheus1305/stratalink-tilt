/**
 * INX Relay  —  ATS (SEC-registered, US-regulated)
 * SEC-registered broker-dealer and ATS for digital securities.
 * Requires INX_API_KEY for live data.
 *
 * Routes:
...
async function fetchLiveDepth(symbol: string): Promise<any> {
  const apiKey = process.env.INX_API_KEY;
  const apiUrl = process.env.INX_API_URL ?? "https://api.inx.co";
  if (!apiKey) throw new Error("INX_API_KEY not configured — requires SEC-registered ATS onboarding");
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), TIMEOUT);
  try {
    const resp = await fetch(
      `${apiUrl}/v1/market/${symbol.toLowerCase()}/depth`,
      { signal: controller.signal, headers: { "X-API-Key": apiKey, "User-Agent": "StrataLink-LIS/1.0" } }
    );
    clearTimeout(timer);
    if (!resp.ok) throw new Error(`INX API HTTP ${resp.status}`);
    return resp.json();
  } catch (e: any) {
    clearTimeout(timer);
    throw new Error(e.name === "AbortError" ? `INX timeout after ${TIMEOUT}ms` : e.message);
  }
}
...
router.get("/spot/depth", async (req: Request, res: Response) => {
  ...
  if (!SUPPORTED_SYMBOLS.includes(symbol)) {
    return res.status(400).json({ ok: false, error: `Symbol ${symbol} not supported on INX. Supported: ${SUPPORTED_SYMBOLS.join(", ")}` });
  }
  ...
  if (process.env.INX_API_KEY) {
    try {
      const data = await fetchLiveDepth(symbol);
      const mid = parseFloat(data?.midPrice ?? "0");
      if (mid > 0) {
    ...
    liveError = "INX_API_KEY not configured";
  }

  if (liveError) console.log(`[INX] Live unavailable (${liveError}), synthetic fallback for ${symbol}`);
  ...
  (snapshot as any).provenance = {
    ...
    note: synthetic ? "Live data requires INX_API_KEY — SEC-registered ATS onboarding required" : undefined,
  };
...
router.get("/health", (_req: Request, res: Response) => {
  res.json({
    ok: !!process.env.INX_API_KEY,
    venue: VENUE,
    venueName: "INX",
    venueType: "ats",
    regulated: true,
    regulator: "SEC",
    status: process.env.INX_API_KEY ? "configured" : "unconfigured",
